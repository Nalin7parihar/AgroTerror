...
from adjust_embedingmodel import Net
...
def save_embeddings_to_pickle(node_mlp_output, gat_output, global_pooled_embeddings):
    # 确保目录存在
    save_dir = "/data/data2/liuxiuqin/JYJ/dataset/update_data/embedding"
    os.makedirs(save_dir, exist_ok=True)

    # 保存路径
    node_mlp_path = os.path.join(save_dir, "node_mlp_embeddings.pkl")
    gat_path = os.path.join(save_dir, "gat_embeddings.pkl")
    gcn_path = os.path.join(save_dir, "gcn_global_pooled_embeddings.pkl")

    # 保存每个嵌入
    with open(node_mlp_path, 'wb') as f:
        pickle.dump(node_mlp_output.detach().cpu().numpy(), f)
    print(f"NodeMLP embeddings saved to {node_mlp_path}")

    with open(gat_path, 'wb') as f:
        pickle.dump(gat_output.detach().cpu().numpy(), f)
    print(f"GAT embeddings saved to {gat_path}")

    with open(gcn_path, 'wb') as f:
        pickle.dump(global_pooled_embeddings.detach().cpu().numpy(), f)
    print(f"GCN global pooled embeddings saved to {gcn_path}")
...
def predict(config, model_path, graph_test_loader, crit_r2, crit_pearson, crit_spearman, excel_path):
    ...
    with torch.no_grad():
        for graph_data in graph_test_loader:
            ...
            #, node_mlp_embeddings, gat_embeddings, global_pooled_embeddings
            print(f"Number of elements in output: {len(output)}")
            # x, node_mlp_output, gat_output, global_pooled_embeddings = output
            # save_embeddings_to_pickle(node_mlp_output, gat_output, global_pooled_embeddings)
            
            label = graph_data.y.cuda()

            if graph_data.batch.size(0) == 0:  # 如果 batch 为空，检查是否有输出
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(model_path)
    excel_path = '/data/data2/liuxiuqin/JYJ/pygall/gat/adjust_output/prediction_kim500foremb2try_results.xlsx'
